...
const MyBookingsPage: React.FC = () => {
  ...
  // ── Correctly categorise bookings ────────────────────────────────
  // upcoming = confirmed + travel date >= today
  // past     = confirmed + travel date < today
  // cancelled = status = cancelled
  const categorised = {
    upcoming:  bookings.filter((b) => b.status === 'confirmed' && b.bus.date >= todayISO),
    past:      bookings.filter((b) => b.status === 'confirmed' && b.bus.date <  todayISO),
                ...
                <div className="p-5">
                  <div className="flex flex-col sm:flex-row gap-4 sm:items-center">
                    ...
                    <div className="flex items-center gap-3 flex-1">
                      <div>
                        <p className="text-lg font-black text-[#1a1a2e]">{booking.bus.departureTime}</p>
                      ...
                      <div>
                        <p className="text-lg font-black text-[#1a1a2e]">{booking.bus.arrivalTime}</p>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-50">
                    ...
                    >
                      ...
                      <ChevronDown className={`w-3.5 h-3.5 transition-transform ${expandedId === booking.id ? 'rotate-180' : ''}`} />
                    </button>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {booking.status === 'confirmed' && booking.bus.date >= todayISO && (
                        <Button
                          variant="ghost"
                          size="sm"
                  ...
                  {expandedId === booking.id && (
                    <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                      <div>
                        <p className="font-bold text-gray-500 text-xs uppercase mb-2">Passengers</p>
                        {booking.passengers.map((p, i) => (
                          <div key={i} className="flex items-center gap-2 mb-1.5">
                            <User className="w-3.5 h-3.5 text-gray-400" />
                            <span className="text-gray-700">{p.name} · {p.age}y · {p.gender}</span>
                          </div>
                        ))}
                      </div>
                      <div>
                        <p className="font-bold text-gray-500 text-xs uppercase mb-2">Journey Points</p>
                        <div className="flex items-start gap-2 mb-2">
                          <MapPin className="w-3.5 h-3.5 text-[#d63031] mt-0.5" />
                          <div>
                            <p className="text-gray-700 font-medium">{booking.boardingPoint.name}</p>
                            <p className="text-xs text-gray-400">Boarding · {booking.boardingPoint.time}</p>
                          </div>
                        </div>
                        <div className="flex items-start gap-2">
                          <MapPin className="w-3.5 h-3.5 text-green-500 mt-0.5" />
                          <div>
                            <p className="text-gray-700 font-medium">{booking.droppingPoint.name}</p>
                            <p className="text-xs text-gray-400">Dropping · {booking.droppingPoint.time}</p>
                          </div>
                        </div>
                      </div>
                      <div>
                        <p className="font-bold text-gray-500 text-xs uppercase mb-2">Contact</p>
                        <p className="text-gray-700 text-xs">{booking.contactEmail}</p>
                        <p className="text-gray-700 text-xs">{booking.contactPhone}</p>
                      </div>
                    </div>
                  )}
                </div>
